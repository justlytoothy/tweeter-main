...
public class MainActivity extends AppCompatActivity implements StatusDialogFragment.Observer, MainActivityPresenter.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (selectedUser.compareTo(Cache.getInstance().getCurrUser()) == 0) {
            ...
            presenter.getIsFollower(Cache.getInstance().getCurrUserAuthToken(), Cache.getInstance().getCurrUser(), selectedUser);
        }
    ...
    @Override
    public void followerCountReceived(int count) {
        followerCount.setText(getString(R.string.followerCount, String.valueOf(count)));
    }

    @Override
    public void isFollowerReceived(boolean isFollower) {
        if (isFollower) {
            followButton.setText(R.string.following);
            followButton.setBackgroundColor(getResources().getColor(R.color.white));
            followButton.setTextColor(getResources().getColor(R.color.lightGray));
    ...
    @Override
    public void updateFollowingOrNot(boolean followingStatus) {
        updateSelectedUserFollowingAndFollowers();
        updateFollowButton(followingStatus);
    }

    @Override
    public void statusPosted() {
        postingToast.cancel();
